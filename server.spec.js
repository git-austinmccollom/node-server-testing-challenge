...
describe("server.js", () => {
  ...
  describe("delete party request", () => {
    test("returns status code 204 deleted", async () => {
      const expectedStatusCode = 204;
      const postRes = await request(server)
        .post("/api/parties")
        .send({ zip: "37207" });
      const postId = postRes.body.id;
      const deleteRes = await request(server).delete(`/api/parties/${postId}`);
      
      expect(deleteRes.status).toEqual(expectedStatusCode);
    });
  });
});
